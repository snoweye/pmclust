...
i.e. the master assignes jobs to workers or itself, and manages communications.
...
Note that the classification id's are exchangible, so the mixing proportion
gives the order of new id. For example, the next result gives
id exchanges rleated to the true id's.
...
has to command other workers to work first befor burning itself.
...
evaulating performace of algorithms developed in the \pkg{pmclust}
...
\end{Command}
More performance comparision can be found in~\citet{Chen2012a}.
